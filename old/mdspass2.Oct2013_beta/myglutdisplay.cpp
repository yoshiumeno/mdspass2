...
extern GLfloat white[], red[], yellow[], gray[], blue[], green[], green2[];
...
void myGlutDisplay( void )
{
  ...
  */
  ...
  if (show_cnt_wall) {
	...
	xx1=atom.rx[i0]*1e9; yy1=atom.ry[i0]*1e9; zz1=atom.rz[i0]*1e9;
	xx2=atom.rx[i1]*1e9; yy2=atom.ry[i1]*1e9; zz2=atom.rz[i1]*1e9;
	xx3=atom.rx[i2]*1e9; yy3=atom.ry[i2]*1e9; zz3=atom.rz[i2]*1e9;
  ...
  if (createconfig_mode>0) {
    if ((iatom_pick>0)&&(iatom_pick<=atom.natom)) {
      for (int i=1; i<atom.natom; i++) {
	if (i==iatom_pick) {
	  memcpy(color[i],yellow,sizeof(GLfloat)*4);
	} else {
