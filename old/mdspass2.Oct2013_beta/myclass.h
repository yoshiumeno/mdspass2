...
double Atom::Enkin()
{
  ...
  for (int i=1; i<=natom; i++) {
        ekin = ekin + 0.5 * atom.wm[i] * ( vx[i]*vx[i] + vy[i]*vy[i] + vz[i]*vz[i] );
  }
  return ekin;
}
...
double Atom::Dist2Closest(int i, int j, int &ix, int &iy, int &iz)
{
  ...
  xx = rx[j] - rx[i]; yy = ry[j] - ry[i]; zz = rz[j] - rz[i];
  xxx = (xx*cell.hmat[0][0]+yy*cell.hmat[1][0]+zz*cell.hmat[2][0])/cell.len[0];
  yyy = (xx*cell.hmat[0][1]+yy*cell.hmat[1][1]+zz*cell.hmat[2][1])/cell.len[1];
  zzz = (xx*cell.hmat[0][2]+yy*cell.hmat[1][2]+zz*cell.hmat[2][2])/cell.len[2];
  ix = 0; iy = 0; iz = 0;
  if (cell.pbcx) {
    if (xxx < -book.frc) {
      xx = xx+cell.hmat[0][0]; yy = yy+cell.hmat[1][0]; zz = zz+cell.hmat[2][0]; ix =  1;
  ...
  if (cell.pbcy) {
    if (yyy < -book.frc) {
      xx = xx+cell.hmat[0][1]; yy = yy+cell.hmat[1][1]; zz = zz+cell.hmat[2][1]; iy =  1;
  ...
  if (cell.pbcz) {
    if (zzz < -book.frc) {
      xx = xx+cell.hmat[0][2]; yy = yy+cell.hmat[1][2]; zz = zz+cell.hmat[2][2]; iz =  1;
...
GEAM::GEAM()
{
  ...
  gff2[0] = 0.702991; gff1[1] = 0.983967; gff1[2] = 2.283863; gff1[3] = 0.282257;
  gff1[4] = 1.966273; gff1[5] = 3.474733; gff1[6] = 0.929508; gff1[7] = 0.921049;
  gff1[8] = 0.200269; gff1[9] = 0.875874; gff1[10]= 1.640098; gff1[11]= 0.661935;
  gff1[12]= 0.122838; gff1[13]= 0.705845; gff1[14]= 0.608587; gff1[15]= 0.928602;
