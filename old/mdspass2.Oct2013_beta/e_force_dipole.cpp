...
void e_force_dipole(int mode)
{ // mode = 0: normal, 1: no E-field update (for debug)
  ...
  while (dp_converged == 0) {
    ...
    for (int i=1; i<=atom.natom; i++) {
	  ...
	  if (rr < dipole.dp_cut) {
	    ...
	    dipole.E_indx[i] -= grad_el*(3*rp*drx-dipole.p_indx[j]); // sign is fixed YU(AK) 2013.10.23
	    ...
	    if (!self) {
	      ...
	      dipole.E_indx[j] -= grad_el*(3*rp*drx-dipole.p_indx[i]); // sign is fixed YU(AK) 2013.10.23
  ...
  for (int i=1; i<=atom.natom; i++) {
	...
	if ((rr < dipole.dp_cut)&&((dipole.dp_alpha[typ1]!=0)||(dipole.dp_alpha[typ2]!=0))) {
	  ...
	  grad_2 = 2 * rp_i * rp_j;
	  atom.epot[i] += value/2.0; atom.epot[j] += value/2.0; // sign is fixed YU 2013.09.30 --> reversed 10.23
	  ...
	  atom.fx[i] -= tmpx; atom.fx[j] += tmpx; // sign is fixed YU 2013.09.30 --> reversed 10.23
	  atom.fy[i] -= tmpy; atom.fy[j] += tmpy;
	  atom.fz[i] -= tmpz; atom.fz[j] += tmpz;
	  // stress
	  tmpx *= rr; tmpy *= rr; tmpz *= rr; // YU 2013.10.18
	  atom.satom[i][0][0] -= tmpx*drx/2.0; // sign fixed YU 2013.10.18 --> reversed 10.23
	  atom.satom[i][0][1] -= tmpx*dry/2.0;
	  atom.satom[i][0][2] -= tmpx*drz/2.0;
	  atom.satom[i][1][1] -= tmpy*dry/2.0;
	  atom.satom[i][1][2] -= tmpy*drz/2.0;
