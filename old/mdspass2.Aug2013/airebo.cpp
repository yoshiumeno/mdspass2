...
void airebo_lj()
{
  ...
  for (int i=1; i<=bre.np; i++) {
	...
	for (int k=jbegin; k<=jend; k++) {
      ...
      // if read from file...
      //double ddd = (bre.ljmax-bre.ljmin)/(double)bre.ljmesh;
      //int idd = (rij-bre.ljmin)/ddd;
      //if (rij>bre.ljmax) { idd = bre.ljmesh; }
      //if (rij<bre.ljmin) { idd = 0; }
      //double vv = bre.lj[idd]; double dvv = bre.ljd[idd];
