...
extern GLfloat red[],blue[],white[],black[];
...
void change_atom_color()
{
  if (color_mode == 0) { // color by species
    set_atom_color();
  } else if (color_mode == 1) { // color by energy
    GLfloat hsv[3], rgb[3];
    double vmin = color_mode_vmin;
    double vmax = color_mode_vmax;
    if (color_mode_auto) {
      vmin = 1.0e20; vmax = -1.0e20;
      for (int i=1; i<=atom.natom; i++) {
	if (vmin > atom.epot[i]/ev) { vmin = atom.epot[i]/ev; }
	if (vmax < atom.epot[i]/ev) { vmax = atom.epot[i]/ev; }
      }
      color_mode_vmin = vmin; color_mode_vmax = vmax;
    }
    for (int i=1; i<=atom.natom; i++) {
      double ep = atom.epot[i]/ev;
      if (ep > vmax) { ep = vmax; }
      if (ep < vmin) { ep = vmin; }
      hsv[0] = 240 - 240 * (ep-vmin) / (vmax-vmin);
      hsv[1] = 1.0;
      hsv[2] = 1.0;
      hsv2rgb(hsv[0], hsv[1], hsv[2], rgb[0], rgb[1], rgb[2]);
      memcpy(color[i],rgb,sizeof(GLfloat)*4);
    }
  } else if (color_mode == 2) { // CSP: Central Symmetric Parameter
    GLfloat hsv[3], rgb[3];
    ...
    for (int i=1; i<=atom.natom; i++) {
      int ni = atom.nneighbor[i]; int mi = std::min(12,ni); mi=floor((double)mi/2)*2;
      //hsv[0] = 240 - 240 * csp(i, mi);
      double c = csp(i, mi);
      if (c>vmax) { c=vmax; } if (c<vmin) { c=vmin; }
      hsv[0] = 240 - 240 * (c-vmin) / (vmax-vmin);
      hsv[1] = 1.0;
      hsv[2] = 1.0;
      hsv2rgb(hsv[0], hsv[1], hsv[2], rgb[0], rgb[1], rgb[2]);
      memcpy(color[i],rgb,sizeof(GLfloat)*4);
  ...
  if ((marked_atom>0)&&(marked_atom<=atom.natom)) {
    if (marked_atom_color==0) {
      memcpy(color[marked_atom],red,sizeof(GLfloat)*4);
    } else if (marked_atom_color==1) {
      memcpy(color[marked_atom],blue,sizeof(GLfloat)*4);
    } else if (marked_atom_color==2) {
      memcpy(color[marked_atom],black,sizeof(GLfloat)*4);
    } else if (marked_atom_color==3) {
      memcpy(color[marked_atom],white,sizeof(GLfloat)*4);
    }
  }
