...
void airebo_lj_test()
{
  ...
  for (int i=1; i<=bre.np; i++) {
    ...
    for (int j=jbegin; j<=jend; j++) {
      ...
      if (wmax==1) { continue; }
      bool calcb = true;
      if (ss(tr(rij,ki,kj))==0) { calcb = false; }
	...
	for (int k=jbegin; k<=jend; k++) {
	  ...
	  double rkj = sqrt(rsq2);
	  double ww = ssp(tc(s3,ki,kk))*ssp(tc(rkj,kj,kk));
	  if (ww > wmax) { wmax = ww; }
	  if (wmax==1) { break; }
	  ...
	  double fc = ssp(tc(rij,ki,kj));
	  double dfc = dssp(tc(rij,ki,kj))/(0.122462048*bre.siglj[ki][kj]);
	...
	for (int l=lbegin; l<=lend; l++) {
	  ...
	  if (kj == 1) {
		...
		if (qj > bre.att) {
	      ...
	      for (int jj=3; jj<=6; jj++) {
		gangle = gangle+bre.spgc[jj][ig]*pow(costh,jj-1);
		gangle1 = gangle1+bre.spgc[jj][ig]*pow(costh,jj-1); }
    ...
    OUT:
      ...
      */
      ...
      // dh-loop 2 for wmax
      if ((jbegin != jend)&&(lbegin != lend)) {
	for (int k=jbegin; k<=jend; k++) { // loop k
	  //if (k == j) { continue; }
	  if (bre.lcheck[k] != 1) { continue; }
	  int kn = bre.list[k];
	  int kk = bre.ktype[kn];
	  ...
	  double xx = bre.cor1[k]-cj[1];
	  double yy = bre.cor2[k]-cj[2];
	  double zz = bre.cor3[k]-cj[3];
	  double xxx = xx*xx+yy*yy+zz*zz;
	  double ww;
	  if (xxx>4.0) {
	    ww = 0;
	  } else {
	    double rkj = sqrt(xx*xx+yy*yy+zz*zz);
	    ww = ssp(tc(rik,ki,kk))*ssp(tc(rkj,kj,kk));
	  }
	  if (ww > wmax) { wmax = ww; }
	  if (wmax==1) { break; }
	  for (int l=lbegin; l<=lend; l++) { // loop l
	    if (bre.lcheck[l] != 1) { continue; }
	    int ln = bre.list[l];
	    //if (ln == i) { continue; }
	    int kl = bre.ktype[ln];
	    ...
	    double rrr = rklx*rklx+rkly*rkly+rklz*rklz;
	    if (rrr>4.0) {
	      ww = 0;
	    } else {
	      double rkl = sqrt(rklx*rklx+rkly*rkly+rklz*rklz);
	      ww = ssp(tc(rik,ki,kk))*ssp(tc(rjl,kj,kl))*ssp(tc(rkl,kk,kl));
	    }
	    if (ww > wmax) { wmax = ww; }
	    if (wmax==1) { break; }
      ...
      double cij = 1 - wmax;
      //double wij = ssp(tc(rij,ki,kj));
      double elj = (ss(tr(rij,ki,kj))*ss(tb(btot,ki,kj))+1-ss(tr(rij,ki,kj)))
	*cij*vlj(rij,ki,kj);
      double delj = (ss(tr(rij,ki,kj))*ss(tb(btot,ki,kj))+1-ss(tr(rij,ki,kj)))
	*cij*dvlj(rij,ki,kj)
      	+dss(tr(rij,ki,kj))/(0.122462048*bre.siglj[ki][kj])
	*(ss(tb(btot,ki,kj))-1)*cij*vlj(rij,ki,kj);

      atom.epot[i]  += elj/2;
      atom.epot[jn] += elj/2;
      //printf("%e %e %e %e %e\n",rij,ss(tr(rij,ki,kj)),elj,cij,vlj(rij,ki,kj));
