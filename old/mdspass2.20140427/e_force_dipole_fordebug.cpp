...
void e_force_dipole()
{
  ...
  //dipole.dp_tol=1.0e-20;
  dipole.val1=0;dipole.val2=0;dipole.val3=0;
  ...
  if (dipole.initialize) {
    ...
    dipole_alloc();
    if (dipole_read_param(dipole.fname) == 0) {
  ...
  for (int i=1; i<=atom.natom; i++) {
	...
	if ((rr < dipole.r_cut[ipair])&&(ipair!=0)) { // calculate short-range forces
	  ...
	  grad /= rr;
	  atom.epot[i] += value/2.0; atom.epot[j] += value/2.0; // OK!!
  ...
  for (int i=1; i<=atom.natom; i++) {
	...
	if ((rr < dipole.dp_cut)&&((dipole.dp_alpha[typ1]!=0)||(dipole.dp_alpha[typ2]!=0))) {
	  ...
	  grad_1 = dipole.charge[typ1] * rp_j * grad_sum * rr;
	  ...
	  */
	  ...
	  /*
	  double value0,grad0,dr=0.0001;
	  buckingham(rr, value0, grad0, dipole.buck_a[ipair], dipole.buck_s[ipair]
	    	     ,dipole.buck_c[ipair]); //Buckingham type
	  buckingham(rr+dr, value, grad, dipole.buck_a[ipair], dipole.buck_s[ipair]
	    	     ,dipole.buck_c[ipair]); //Buckingham type
	  //printf("%e %e %e\n",value0,(value-value0)/dr,grad0);
	  double value_el0,grad_el0,ggrad_el0;
	  elstat_shift(rr, dipole.dp_kappa, value_el0, grad_el0, ggrad_el0); //tail-functions
	  elstat_shift(rr+dr, dipole.dp_kappa, value_el, grad_el, ggrad_el); //tail-functions
	  //printf("%e %e %e\n",grad_el0,(grad_el-grad_el0)/dr,ggrad_el0*rr);
	  */
	  //value=rp_j * value_sum*rr;
	  value=rp_j * rr;
	  grad=grad_sum*rr;
	  ...
	  //atom.epot[i] -= value;
	    //}
	  //tmpx = rp_j* drx * grad *rr + value_sum*dipole.p_indx[j];
	  //tmpy = rp_j* dry * grad *rr + value_sum*dipole.p_indy[j];
	  //tmpz = rp_j* drz * grad *rr + value_sum*dipole.p_indz[j];
	  tmpx = dipole.p_indx[j];
	  tmpy = dipole.p_indy[j];
	  tmpz = dipole.p_indz[j];
	  //if (i==1){
	  atom.fx[i] -= tmpx; atom.fy[i] -= tmpy; atom.fz[i] -= tmpz;
	  atom.fx[j] += tmpx; atom.fy[j] += tmpy; atom.fz[j] += tmpz;
	  //}
	  if (i==1) {
	    //atom.epot[i] -= rp_j*rr;
	    //atom.fx[i] -= dipole.p_indx[j];
	  }
	  //if (i==1) { dipole.val1 -= rp_j*rr; }
	  //if (i==1) { dipole.val2 -= dipole.p_indx[j]; }
	  dipole.val1 += dipole.p_indx[i];
