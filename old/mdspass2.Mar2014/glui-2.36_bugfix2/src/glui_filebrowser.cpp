...
int comp_func(const void *a, const void *b) {
  return( strcmp( (char *)(((File_List *)a)->file_name),
    (char *)(((File_List *)b)->file_name) ) );
}
//
...
void GLUI_FileBrowser::dir_list_callback(GLUI_Control *glui_object) {
  ...
  if (this_item >= 0) { //Y.Umeno May18 2012
    selected = list->get_item_ptr( this_item )->text.c_str();
    //if (selected[0] == '/' || selected[0] == '\\') {
    if (selected[strlen(selected)-1] == '/' || selected[0] == '\\') { //Y.Umeno May18 2012
      if (me->allow_change_dir) {
#ifdef __GNUC__
        //chdir(selected+1);
        chdir(selected); //Y.Umeno May18 2012
#endif
#ifdef _WIN32
...
//Y.Umeno
      dir = opendir(d);
      file_num = i;
      file_list = (File_List *)calloc(file_num, sizeof(File_List));
      sprintf(max_column,"%d",file_num);
      i = 0;
      while ((dirp = readdir(dir)) != NULL)   /* open directory     */
      {
        if (!lstat(dirp->d_name,&dr) && S_ISDIR(dr.st_mode)) /* dir is directory   */
          ...
          item = dirp->d_name;
        strcpy( (file_list+i)->file_name, item.c_str());
        (file_list+i)->num = i;
        i++;
      }
      closedir(dir);
      ...
      //  fprintf(stdout, "%*d %s\n",strlen(max_column),
      //    (file_list+ii)->num, (file_list+ii)->file_name); }
      qsort(file_list, file_num, sizeof(File_List), comp_func);
      //fprintf(stdout,"\n********After sort filename*********\n\n");
      list->delete_all();
      for (int ii=0; ii<file_num; ii++) {
        //fprintf(stdout, "%*d %s\n",strlen(max_column),
        //  (file_list+ii)->num, (file_list+ii)->file_name);
        list->add_item(ii,(file_list+ii)->file_name);
       }
//
    }
  }
